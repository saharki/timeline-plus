 ...
 */
    ...
    const queue = [
      ...
      () => {
        // if (this.options.clusterItems) {
          this.items = this._clusterItems(this.rawItems)
        // }
      },
  ...
  _clusterItems(items) {
    ...
    if (items instanceof ItemSet) {
      ...
      clusteredItemsArr = util.toArray(...items.items)
    }
    else {
      clusteredItemsArr = [...util.toArray(items)];
    }

    for (var i = 0; i < clusteredItemsArr.length; i++) {  // eslint-disable-line no-redeclare
      for (let j = i + 1; j < clusteredItemsArr.length; j++) {
        if (clusteredItemsArr[i].isColliding(clusteredItemsArr[j])) {
          const mergedItem = Item.mergeItems(clusteredItemsArr[i], clusteredItemsArr[j])
          if(mergedItem) {
    ...
    clusteredItemsArr.forEach((item) => {
  ...
  _removeFromVisibleItems(item) {
    const itemIndex = this.visibleItems.findIndex((visibleItem) => item.id === visibleItem.id)
    //  Check the item is actually in this subgroup. How should items not in the group be handled?
    if (itemIndex >= 0) {
