...
class Item {
   ...
   */
  isColliding(otherItem) {
    const rtl = this.options.rtl;
    const margin = this.options.margin.item

    if (rtl) {
      return  ((this.right - margin.horizontal + EPSILON)  < (otherItem.right + otherItem.width) &&
      (this.right + this.width + margin.horizontal - EPSILON) > otherItem.right &&
      (this.top - margin.vertical + EPSILON)              < (otherItem.top + otherItem.height) &&
      (this.top + this.height + margin.vertical - EPSILON)   > otherItem.top);
    } else {
      return ((this.left - margin.horizontal + EPSILON)   < (otherItem.left + otherItem.width) &&
      (this.left + this.width + margin.horizontal - EPSILON) > otherItem.left &&
      (this.top - margin.vertical + EPSILON)              < (otherItem.top + otherItem.height) &&
      (this.top + this.height + margin.vertical - EPSILON)   > otherItem.top);
    }
  }
   ...
   * @return {boolean}        true if a and b collide, else false
   */
  static isColliding(a, b, margin, rtl) {
    if (rtl) {
   ...
   */
  static mergeItems(baseItem, otherItem) {
    
    const typesConstructors = {}
    util.toArray(types).forEach((typeConstructor) => {
    ...
    const data = util.extend({}, baseItem.data);
    data.content = 'merged' //baseItem.content + ' merged!';
    const mergedItem = new typesConstructors.base(data, baseItem.conversion, baseItem.options)
