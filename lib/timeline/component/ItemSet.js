 ...
 */
  ...
  _clusterItems(items) {
    ...
    if (items instanceof ItemSet) {
      ...
      clusteredItems = [ ...items.items ]
    }
    else {
      clusteredItems = [ ...items ];
    }

    for (var i = 0; i < clusteredItems.length; i++) {  // eslint-disable-line no-redeclare
      for (let j = i + 1; j < clusteredItems.length; j++) {
        if (clusteredItems[i].isColliding(clusteredItems[j])) {
          clusteredItems[i] = Item.mergeItems(clusteredItems[i], clusteredItems[j])
