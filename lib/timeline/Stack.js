...
export function stack(items, margin, force, shouldBailItemsRedrawFunction) {
  ...
  for (var i = 0; i < items.length; i++) {  // eslint-disable-line no-redeclare
    ...
    if (items[i].stack && items[i + 1] && items[i + 1].top === null) {
      ...
      const itemEndingSoonest = lastItemByLevels[0]
      let currentLevelLastItem = lastItemByLevels[currentStackLevel]

      // Check if the first row has enough space for this item,
      // else check if the current row's last item don't collide with the item
      if (item && item !== itemEndingSoonest && item.stack && itemEndingSoonest && itemEndingSoonest.top !== null && !collision(itemEndingSoonest, item, margin.item, item.options.rtl)) { 
        currentStackLevel = 0; 
      }
      else if (item && item !== currentLevelLastItem && item.stack && currentLevelLastItem && currentLevelLastItem.top !== null) {
        //adjust item top to current level's top value
        item.top = currentLevelLastItem.top

        if (collision(currentLevelLastItem, item, margin.item, item.options.rtl)) {
          currentStackLevel = currentStackLevel + 1;
          maxStackLevel = (currentStackLevel > maxStackLevel) ? currentStackLevel : maxStackLevel;
  
          item.top = currentLevelLastItem.top + currentLevelLastItem.height + margin.item.vertical;
        }
      }

      lastItemByLevels[currentStackLevel] = item
    }
  }
  return shouldBail;
}
