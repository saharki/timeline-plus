...
export function stack(items, isOrdered, margin, force, shouldBailItemsRedrawFunction) {
  ...
  for (var i = 0; i < items.length; i++) {  // eslint-disable-line no-redeclare
    ...
    if (item && item.stack) {
      ...
      const firstItemToCollide = collideQueue[0]
      const lastItem = collideQueue[collideQueue.length - 1]

      // check if the item collides with the soonest item to end
      // else, increase item top.
      if (!item.stack) {
        item.top = margin.axis;
      }
