...
export function stack(items, margin, force, shouldBailItemsRedrawFunction) {
  ...
  for (var i = 0; i < items.length; i++) {  // eslint-disable-line no-redeclare
    ...
    if (items[i].stack && items[i + 1] && items[i + 1].top === null) {
      ...
      // initialize top position for the next item      
      if (nextItem && nextItem.top == null) {
        nextItem.top = margin.axis; 
      }

      if (firstItemToCollide && !collision(nextItem, firstItemToCollide, margin.item, firstItemToCollide.options.rtl)) {
        firstItemToCollide = null; 
      }
      else {
        nextItem.top = item.top; //assign for collision test
      
        if (nextItem && item.top !== null && nextItem !== item && nextItem.stack && collision(item, nextItem, margin.item, nextItem.options.rtl)) {
          firstItemToCollide = item; 
          nextItem.top = item.top + item.height + margin.item.vertical;
          continue;
        }
        
        nextItem.top = margin.axis; //restore to value when there is no collision
      }
    }
  }
  return shouldBail;
